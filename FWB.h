...
//
// Further edited by Nikolichnik (nikola7sto@gmail.com)
//
...
private:
  ...
  int DblClickDelay;					// Max period between clicks for a double click event (in ms)
  int LongPressDelay;					// Hold period for a long press event (in ms)
  int VLongPressDelay;					// Hold period for a very long press event (in ms)
    
  // Variables //
  
  boolean _state;						// Value read from button
  boolean _lastState;					// Last value of button state
  boolean _dblClickWaiting;				// whether we're waiting for a double click (down)
  boolean _dblClickOnNextUp;			// whether to register a double click on next release, or whether to wait and click
  boolean _singleClickOK;				// whether it's OK to do a single click
  ...
  boolean _longPressHappened;			// whether or not the hold event happened already
  boolean _vLongPressHappened;			// whether or not the long hold event happened already
